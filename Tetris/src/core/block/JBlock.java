...
class JBlock extends AbstractBlock {
	...
	public JBlock(StaticLevel level_) {
		super(level_);
	}
	...
	public void init() {
		//Define material
		try {
			m_material = new Image("sprites/block_blue.png");
	...
	protected ElementaryBlock[] getNextPosition() {
		...
		switch(m_angle) {
			...
			case 0:
				...
				precomputedBlocks[2].setX(m_xGrid);
				precomputedBlocks[2].setY(m_yGrid + 2*m_level.CELL_SIZE);
			...
			case 3:
				...
				precomputedBlocks[2].setX(m_xGrid + 2*m_level.CELL_SIZE);
